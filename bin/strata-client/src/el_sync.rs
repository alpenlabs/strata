...
pub fn sync_chainstate_to_el(
    ...
    let earliest_idx = chain_state_db.get_earliest_state_idx()?;
    let latest_idx = chain_state_db.get_last_state_idx()?;

    debug!(?earliest_idx, ?latest_idx, "search for last known idx");

    let sync_from_idx = find_last_match((earliest_idx, latest_idx), |idx| {
        let Some(chain_state) = chain_state_db.get_toplevel_state(idx)? else {
    ...
    for idx in sync_from_idx..=latest_idx {
        ...
        let Some(l2block) = l2_db.get_block_data(block_id)? else {
