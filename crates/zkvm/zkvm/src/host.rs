...
pub trait ZkVmHost: Send + Sync + Clone + Display + 'static {
    ...
    type ProofImpl: Into<ProofReceipt> + From<ProofReceipt>;
    ...
    fn verify_inner(&self, proof: &Self::ProofImpl) -> ZkVmResult<()>;

    /// Verifies the [`ProofReceipt`]
    fn verify(&self, proof: &ProofReceipt) -> ZkVmResult<()> {
        self.verify_inner(&proof.clone().into())
    }
}
