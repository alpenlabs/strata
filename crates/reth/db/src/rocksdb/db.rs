...
impl<DB: SchemaDBOperations> WitnessProvider for WitnessDB<DB> {
    fn get_block_witness(&self, block_hash: B256) -> DbResult<Option<ElBlockStfInput>> {
        ...
        let parsed: Option<ElBlockStfInput> = raw
...
mod tests {
    ...
    use strata_proofimpl_evm_ee_stf::{ElBlockStfInput, ElBlockStfOutput};
    ...
    fn del_and_get_block_data() {
        ...
        let received_witness = db.get_block_witness(block_hash);
        assert!(matches!(received_witness, Ok(Some(ElBlockStfInput { .. }))));
