...
impl<T: Clone> SlotState<T> {
    ...
    pub async fn get_async(&self) -> DbResult<Option<T>> {
        match self {
            Self::Ready(v) => Ok(Some(v.clone())),
            Self::Pending(ch) => {
    ...
    pub fn get_blocking(&self) -> DbResult<Option<T>> {
        match self {
            Self::Ready(v) => Ok(Some(v.clone())),
            Self::Pending(ch) => {
                // When we see this log get triggered and but feels like the corresponding fetch is
...
impl<K: Clone + Eq + Hash, V: Clone> CacheTable<K, V> {
    ...
    pub async fn get_or_fetch_async(
        &self,
        k: &K,
        fetch_fn: impl Fn() -> DbRecv<Option<V>>,
    ) -> DbResult<Option<V>> {
    ...
    pub fn get_or_fetch_blocking(
        &self,
        k: &K,
        fetch_fn: impl Fn() -> DbResult<Option<V>>,
    ) -> DbResult<Option<V>> {
...
mod tests {
    ...
    async fn test_basic_async() {
        ...
        let res = cache
            .get_or_fetch_async(&42, || {
                ...
                tx.send(Ok(Some(10))).expect("test: send init value");
                rx
            })
            .await
    ...
    fn test_basic_blocking() {
        ...
        let res = cache
            .get_or_fetch_blocking(&42, || Ok(Some(10)))
            .expect("test: cache gof");
        assert_eq!(res, Some(10));
