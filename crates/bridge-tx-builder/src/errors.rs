...
pub enum BridgeTxBuilderError {
    ...
    #[error("could not build deposit transaction caused by: {0}")]
    DepositTransaction(#[from] DepositTransactionError),

    /// Error building the Deposit Transaction.
    #[error("could not build cooperative withdrawal transaction caused by: {0}")]
    ...
    #[error("could not build taproot address caused by: {0}")]
