...
use strata_primitives::{
    buf::Buf32,
    l1::{L1BlockManifestWithScanRule, L1TxRef},
};
...
mod tests {
    ...
    fn insert_block_data(
        ...
        db: &L1Db,
        num_txs: usize,
    ) -> (L1BlockManifestWithScanRule, Vec<L1Tx>, CompactMmr) {
        let mut arb = ArbitraryGenerator::new();

        // TODO maybe tweak this to make it a bit more realistic?
        let mf: L1BlockManifestWithScanRule = arb.generate();
    ...
    fn test_insert_into_non_empty_db() {
        ...
        for invalid_idx in invalid_idxs {
            ...
            let res = db.put_block_data(
                invalid_idx,
                ArbitraryGenerator::new().generate::<L1BlockManifestWithScanRule>(),
                txs,
            );
