...
mod tests {
    ...
    fn insert_block_data(
        ...
        let res = db.set_canonical_chain_entry(height, mf.block_hash());
        assert!(res.is_ok(), "put should work but got: {}", res.unwrap_err());

        // Insert mmr data
        db.put_mmr_checkpoint(mf.block_hash(), mmr.clone()).unwrap();
    ...
    fn test_put_mmr_checkpoint_valid() {
        ...
        let res = db.put_mmr_checkpoint(mf.block_hash(), mmr);
        assert!(res.is_ok());
    }
    ...
    fn test_get_mmr() {
        ...
        let observed_mmr = db.get_mmr(mf.block_hash()).unwrap();
    ...
    fn test_get_txs_fancy() {
        ...
        for i in 0..total_num_blocks {
            ...
            l1_txs.push((mf.block_hash(), block_txs));
        }
