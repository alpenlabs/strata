...
impl SP1Host {
    pub fn prove_v2<T: serde::Serialize>(
        &self,
        inputs: (VerifcationKey, Proof, T),
        ...
        // Setup the I/O
        let mut stdin = SP1Stdin::new();
        let (vk_ser, proof_ser, input) = inputs;

        let proof: SP1ProofWithPublicValues = bincode::deserialize(proof_ser.as_bytes())?;
        let vkey: SP1VerifyingKey = bincode::deserialize(&vk_ser.0)?;

        stdin.write(&vkey.hash_u32());
        stdin.write(&proof.public_values.to_vec());

        let SP1Proof::Compressed(proof) = proof.proof else {
            panic!("can only do the compressed proof")
        };
    ...
    // pub fn get_proof_composition_vals(
    //     proof: Proof,
    //     verification_key: VerifcationKey,
    // ) -> anyhow::Result<([u32; 8], Vec<u8>)> {
    //     let proof: SP1ProofWithPublicValues = bincode::deserialize(proof.as_bytes())?;
    //     let verification_key: SP1VerifyingKey = bincode::deserialize(&verification_key.0)?;
    //     let proof = proof.proof;
    //     Ok((verification_key.hash_u32(),proof, proof.public_values.to_vec()))
    // }
}
