...
use express_proofimpl_btc_blockspace::filter::{DepositRequestData, ForcedInclusion};
...
pub fn process_checkpoint_proof(
    ...
    let prev_checkpoint = l1_batch
        ...
        .map(|prev_state_update| {
            ...
            assert_eq!(
                initial_l1_state_hash, prev_state_update.l1_state_hash,
                "L1 state mismatch"
            );
            assert_eq!(
                initial_l2_state_hash, prev_state_update.l2_state_hash,
                "L2 state mismatch"
            );
