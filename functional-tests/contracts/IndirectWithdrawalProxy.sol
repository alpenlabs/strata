...
contract WithdrawCaller {
    ...
    event Withdraw(address indexed sender, uint256 amount, bytes bosd);
    ...
    /**
      ...
      */
    function withdraw(bytes calldata bosd) external payable {
        (bool success, ) = PRECOMPILE_BRIDGEOUT_ADDRESS.call{value: msg.value}(bosd); // Calls precompile with ETH
        require(success, "Precompile call failed");

        emit Withdraw(msg.sender, msg.value, bosd);
    }

    /**
      * Contract will have sBTC to bridgeOut
      */
    function withdrawWithOwnBalance(bytes calldata bosd) external {
        ...
        emit Withdraw(msg.sender, BRIDGE_OUT_FIXED_VALUE, bosd);
    }

    /**
      * Contract will just call precompile. Always expected to fail
      */
    function withdrawWithoutBalance(bytes calldata bosd) external {
        (bool success, ) = PRECOMPILE_BRIDGEOUT_ADDRESS.call(bosd); // Calls precompile without ETH
        require(success, "Precompile call failed");

        emit Withdraw(msg.sender, 0, bosd);
    }
